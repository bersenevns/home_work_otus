...
Мы видим, что на Leaf-1 известно 2 соседа VTEP, куда он будет направлять BUM-трафик в случае необходимости. Также мы видим, что к нам пришли маршруты с mac-адресами хостов, которые добавлены в локальную таблицу mac-адресов.

Соседство BGP установлено, маршруты ipv4 приходят, связь до loopback есть, образуются соседи VTEP динамически, они обмениваются маршрутами type-2, type-3, таблицы мак-адресов заполняются, каждый Leaf знает о канальных адресах хостов, которые расположены на других Leaf. VXLAN L2VNI работает, ping между хостами есть. 
Задача выполнена!
